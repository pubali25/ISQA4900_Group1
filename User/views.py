...
from OnlineHobbies.models import OnlineHobbies, OnlineClassVideoPage, OnlineClassEnroll


from OnlineHobbies.models import OnlineHobbies
...
def online_hobby_list(request):
    online = OnlineHobbies.objects.filter()
...
def online_onclick_getstarted(request,pk):
    onlineclassvideo=OnlineClassVideoPage.objects.all().filter(online_hobby_name_id=pk).first()
...
def add_to_cart(request, **kwargs):
    ...
    hobbies = Hobbies.objects.filter(hobby_name=kwargs.get('item_id', "")).first()
    ...
    order_item, status = CartItem.objects.get_or_create(hobbies= hobbies)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if NewsUsers.objects.filter(email=instance.email).exists():
                messages.error( request,'You have already been added to our subscription list')
                ...
                subject = "Welcome to MyPersonalSpace"
                from_email = settings.EMAIL_HOST_USER
                to_email = instance.email
                with open(settings.BASE_DIR + "/templates/subscription_mail.txt") as f:
                    signup_message = f.read()
                message = EmailMultiAlternatives(subject=subject, body=signup_message, from_email=from_email,
                                                 to=[to_email])
                html_template = get_template("subscription_mail.html").render()
                message.attach_alternative(html_template, "text/html")
                message.attach_file(settings.MEDIA_ROOT + "/User/images/mypersonalspace-2.png", mimetype="image/png")
                message.attach_file(settings.MEDIA_ROOT + "/User/images/mypersonalspace-1.png", mimetype="image/png")
                message.send()
                return redirect(reverse_lazy('about'))
        else:
            form = NewsUserForm()
...
def online_class_enroll(request,pk):
    ...
    print(pk)
    enrollment, status = OnlineClassEnroll.objects.get_or_create(enrollment_status= True, username_id=request.user.id, online_hobby_name_id=pk)
    storage = messages.get_messages(request)
    storage.used = True
...
def order_confirm(request, **kwargs):
    ...
    subject = 'Thank you for your reservation at MyPersonalSpace!'
