...
class OnlineHobbies(models.Model):
    ...
    def __str__(self):
        return str(self.online_hobby_name)
...
class OnlineClassEnroll(models.Model):
    online_hobby_name=models.ForeignKey(OnlineHobbies, on_delete=models.CASCADE, related_name='OnlineClassEnroll', default=3)
    ...
    def __str__(self):
        return '{0} enrolled by \t {1}  on the date  {2}'.format(self.online_hobby_name,self.username,self.created_date)
...
# this model provides the admin/instructor to register new hobbies with below fields - Apr 9 2020
class OnlineClassVideoPage(models.Model):
    online_hobby_name=models.ForeignKey(OnlineHobbies, on_delete=models.CASCADE, related_name='OnlineClassVideoPage')
